...
+ (void)mfzUserRegisterEmail:(NSString *)email Resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    [[MyFiziqSDK shared].user registerWithEmail:email completion:^(NSError *err, NSString *userid, NSString *clashid) {
        if (err) {
            if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedRegistration userInfo:@{NSLocalizedDescriptionKey:@(err.code)}]);
        } else {
...
+ (void)mfzUserLoginEmail:(NSString *)email Resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    [[MyFiziqSDK shared].user logInWithEmail:email completion:^(NSError *err) {
        if (err) {
            if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedLogin userInfo:@{NSLocalizedDescriptionKey:@(err.code)}]);
        } else {
...
+ (void)mfzUserLogoutResolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    [[MyFiziqSDK shared].user logOutWithCompletion:^(NSError *err) {
        if (err) {
            if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedLogout userInfo:@{NSLocalizedDescriptionKey:@(err.code)}]);
        } else {
            if (resolve) resolve(nil);
        }
    }];
...
+ (void)mfzUserSetGender:(NSString *)gender resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    ...
    if (!gender || (![gender isEqualToString:@"male"] && ![gender isEqualToString:@"female"])) {
        if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedSetGenderBadParam userInfo:@{NSLocalizedDescriptionKey:@"Param must be either 'male' or 'female'."}]);
    } else {
...
+ (void)mfzUserSetEmail:(NSString *)email resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    // Check params
    if (!email) {
        if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedSetEmailBadParam userInfo:nil]);
    } else {
...
+ (void)mfzUserSetMeasurementPreference:(NSString *)pref resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    if (!pref || (![pref isEqualToString:@"metric"] && ![pref isEqualToString:@"imperial"])) {
        if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedSetMeasurementPrefBadParam userInfo:@{NSLocalizedDescriptionKey:@"Param must be either 'metric' or 'imperial'."}]);
    } else {
...
+ (void)mfzUserSetWeightKg:(float)weightKg resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    ...
    if (weightKg < 17 || weightKg > 300) {
        if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedSetWeightParamInvalid userInfo:@{NSLocalizedDescriptionKey:@"Param value must be between 17 and 300 kg."}]);
    } else {
...
+ (void)mfzUserSetHeightCm:(float)heightCm resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject {
    ...
    if (heightCm < 50 || heightCm > 300) {
        if (reject) reject([NSError errorWithDomain:RNMFZUSER_ERR_DOMAIN code:RNMFZUserErrorFailedSetHeightParamInvalid userInfo:@{NSLocalizedDescriptionKey:@"Param value must be between 50 and 300 cm."}]);
    } else {
